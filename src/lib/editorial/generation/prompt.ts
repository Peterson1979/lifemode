...
export function buildGenerationPrompt(request: GenerationRequest): GenerationPromptPayload {
  ...
  const systemPromptParts: string[] = [
    ...
    '3. Opening & Conclusion Directives:',
    '   - Do NOT begin articles with a generic "Introduction" or "Overview" heading. Start directly with the actual subject, observation, situation, question, or useful context.',
    '   - Do not restate the article title as the opening sentence unless there is a genuine editorial reason.',
    '   - Craft an engaging, immersive opening that gets straight to the subject, and a thoughtful, actionable conclusion synthesizing long-term lifestyle habits without generic filler.',
    '4. Natural Editorial Writing & Zero AI Clichés (Strictly Enforced):',
    ...
    '   B. Direct, organic openings: Begin directly with the subject. Never open with generic filler about modern life, changing times, today\'s fast-paced world, or the importance of the topic.',
    '   C. Zero predictable rhetorical formulas: Avoid template structures such as:',
    ...
    '      - "In conclusion..."',
    '      - "Here\'s why..." / "Let\'s dive into..."',
    '   D. No artificial sophistication or inflated voice: Never make simple subjects sound grandiose, philosophical, or ceremonial. Write with calm clarity.',
    '   E. Natural sentence & paragraph variety: Vary paragraph length and sentence structure naturally. Avoid monotonous, predictable cadences.',
    '   F. Organic article architecture: Build an organic structure tailored to the actual topic. Do not force a fixed number of sections or a rigid formulaic template.',
    '   G. Clean, natural punctuation: Avoid excessive use of em dashes, semicolons, rhetorical questions, and overly polished marketing language.',
    '   H. Avoid repetitive three-part lists: Do not force "three-part" lists or triads when the subject does not naturally call for them.',
    '   I. No meta-process references: Never use phrases that explicitly reveal or discuss the article-generation process.',
    '   J. Professional lifestyle editor standard: The writing must feel like it was written and edited by an experienced human lifestyle editor who knows the subject deeply, not assembled from a generic template.',
  ...
  userPromptParts.push(
    ...
    `- Opening Context (~15% / ~${Math.round(targetWords * 0.15)} words): Open immediately with the actual subject, observation, situation, or useful context without any generic "Introduction" heading, boilerplate preamble, or title restatement.`,
    `- Core Major Sections (~70% / ~${coreWordsTarget} words total across ${numOutlineSections} sections, ~${wordsPerSection} words each): Thoroughly develop each section with multiple detailed paragraphs, concrete methodologies, specific nuances, and practical advice under organic H2 and H3 headings.`,
    `- Conclusion & Practical Takeaways (~15% / ~${Math.round(targetWords * 0.15)} words): Synthesize the insights into enduring lifestyle practices and actionable takeaways without formulaic summary transitions.`,
